...
## Task 1 — post-review resolution (11.08.2026)

Task reviewer found 2 Important, both plan-mandated (not implementer error):

1. **Неполный список известных неточных якорей.** План называл только `04-classifier.md` (2
   ссылки на `#сколько-сегментов-выделять-на-одном-видео` для фактов, переехавших под новый
   заголовок). Ревьюер нашёл ещё 3: `07-faq.md:46`, `09-disputed-points.md:47`,
   `10-qa-log.md:928` — тот же паттерн (ссылка ведёт на верную страницу, но не точно к абзацу).
   **Решение пользователя:** оставить как есть, список расширен здесь для памяти; поправить при
   правке соответствующих файлов в следующих волнах (07/09 — волна 3+; 10-qa-log.md — вне объёма
   дедупа вообще, архив).
2. **Эмодзi 🚫 в обычном абзаце, не в пункте списка** (`## Когда видео помечается «Битое»`,
   первая строка тела). Формально нарушает букву ограничения «эмодзи только в пунктах списка»,
   но не нарушает его смысл (защита якорей — эмодзи не в заголовке). **Решение пользователя:**
   оставить как есть.
