...
- Работаем в ветке `feature/stage2-testing-page` (создана из `main`), без worktree. Каждая задача коммитит и пушит в эту ветку (пуш в неё **не** триггерит деплой GitHub Pages — `deploy.yml` слушает только `main`). В конце — один PR в `main`.
...
- [ ] **Step 1b: Убрать двойную эмодзи в левом меню (findings из ревью Task 1)**

В `site/theme/quiz.css` удалить правило-псевдоэлемент целиком:
```css
.md-nav__link[href$="/manual-2-etap/07-testirovanie/"]::before {
  content: "🧪 ";
}
```
Эмодзи `🧪` уже есть в самом тексте пункта меню (`- 🧪 Тестирование по 2 этапу:` в обоих
конфигах) и показывается и во вкладке сверху, и в сайдбаре — `::before` добавлял вторую.
