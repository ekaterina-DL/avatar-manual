...
## ✅ Третье и четвёртое исправление, найденные финальным ревью после Task 9 (класс «работает
## локально, ломается на чистом клоне»)

Task 9 много раз запускал `mkdocs build --strict` и всегда получал exit 0 — но каждый раз в
**той же самой** рабочей копии, где годами накапливались файлы, которые в git не попадают.
Финальный ревью всей ветки (после Task 9) склонировал репозиторий в чистую временную папку —
то есть воспроизвёл ровно то, что делает `actions/checkout` в GitHub Actions — и получил
`exit code 1` на обоих профилях сборки. Раз воспроизведено на настоящем чистом клоне, а не
предположение — это и была причина первого красного запуска workflow.

**Находка 3: `theme.custom_dir: theme/overrides` указывает на директорию, которой нет в git.**
Git не отслеживает пустые директории — `site/theme/overrides/` создана в Task 2 пустой (под
будущие Phase 2 переопределения шаблонов, которых пока нет) и никогда не коммитилась. Локально
папка на диске осталась (создана один раз командой `mkdir`), поэтому все локальные сборки её
находили. На чистом клоне (и в GitHub Actions) папки физически нет — MkDocs падает уже на
валидации конфига: `ERROR - The path set in custom_dir (...) does not exist`. **Исправление:**
закоммичен `site/theme/overrides/.gitkeep` — пустой файл-плейсхолдер, чтобы git начал отслеживать
директорию.

**Находка 4: три страницы мануала ссылаются на `_sources-log.md`, который сознательно не
публикуется (см. `.gitignore` из Task 1).** Ссылки `[_sources-log.md](_sources-log.md)` в
`manual-2-etap/11-example-library.md`, `manual-3-etap/00-overview.md`,
`manual-3-etap/03-common-mistakes.md` — рабочие и корректные, когда мануал читают как обычный
markdown вне сайта (файл `_sources-log.md` реально существует на диске у каждого, кто его туда
положил, просто не в git). Локально MkDocs всегда находил файл на диске (сборка обходит саму
файловую систему, а не `git ls-files`) и не ругался. На чистом клоне файла нет вообще — ссылка
становится по-настоящему битой, и `--strict` валит сборку тремя `WARNING`. Менять сами файлы
мануала нельзя (Global Constraints — они должны оставаться валидным markdown вне сайта, а вне
сайта `_sources-log.md` реально есть и ссылка должна работать). **Исправление:** третий хук
`site/hooks/neutralize_excluded_links.py` — тем же приёмом, что и `wrap_source_tags.py`
(`on_page_markdown`, не трогает сам файл мануала), превращает на лету markdown-ссылки на
`_sources-log.md` / `_raw-sources/...` / `voprosy-zakazchiku.md` в обычный текст без ссылки —
целятся только эти три сознательно не публикуемых пути, обычные рабочие ссылки хук не трогает.

**Урок на будущее, зафиксированный в самом фиксе:** любая повторная проверка `--strict` после
этого делается через `git clone` во временную папку, а не в рабочей копии — иначе тот же класс
бага («есть у меня на диске, нет в git») снова незаметно проскочит.

## ✅ Пятое исправление: домашняя страница не открывалась по адресу `<username>.github.io/avatar-manual`

После того как GitHub Actions стал зелёным, живой сайт по корневому адресу показывал не
домашнюю страницу, а сломанную 404-страницу (гигантская нестилизованная чёрная иконка вместо
текста) — нашла и воспроизвела на самом живом сайте (`curl` по реальным URL), не догадкой.

**Причина, часть 1 — домашняя страница физически не в корне.** `docs_dir: ..` — это корень
проекта, а `site/home/index.md` лежит на два уровня ниже него — поэтому у неё генерируется URL
`/avatar-manual/site/home/`, а не `/avatar-manual/`. У MkDocs нет автоматического «первый пункт
nav становится корнем сайта» — корнем становится только файл `index.md`, лежащий прямо в
`docs_dir` (а туда мы намеренно не клали ничего, кроме исключённого `README.md`). Значит
корневой адрес всегда 404-ился, с самого первого деплоя — просто это никто не проверял: все 9
задач проверяли только `mkdocs build --strict` (успешную сборку), но ни разу — что именно
показывает браузер по адресу сайта из `site/PLAN.md` (`<username>.github.io/avatar-manual`).

**Причина, часть 2 — у автосгенерированной `404.html` были битые абсолютные пути к CSS/JS.**
Обычные страницы мануала используют вычисляемые относительные пути (`../../assets/...`) и
работают корректно. Но у `404.html` MkDocs не может вычислить «глубину» страницы (на неё можно
попасть с любого несуществующего адреса), поэтому она использует абсолютные пути от корня
домена — а без `site_url` в конфиге эти пути получаются `/assets/...` вместо
`/avatar-manual/assets/...`, то есть ведут мимо GitHub Pages project-адреса на несуществующий
путь прямо на `ekaterina-dl.github.io/` — отсюда полностью нестилизованная страница.

**Исправление:**
- Добавлен `site_url: https://ekaterina-dl.github.io/avatar-manual/` в `site/mkdocs.yml` —
  даёт MkDocs правильную базу для абсолютных путей на `404.html` (и для sitemap.xml).
- Домашняя страница перенесена из `site/home/index.md` в `index.md` прямо в корне проекта
  (единственное место, которое MkDocs производит в корневой `index.html` сайта) — внутренние
  ссылки поправлены (без `../../`, так как теперь файл лежит рядом с `manual-2-etap/` и
  `manual-3-etap/`, а не на два уровня ниже). `site/mkdocs.yml` (`nav:` → `Главная`) и
  `site/mkdocs-pdf.yml` (`plugins: print-site: exclude:`) обновлены на новый путь `index.md`.
  Папка `site/home/` больше не существует.

**Урок на будущее:** после любой правки, влияющей на структуру URL, проверять не только
`mkdocs build --strict`, но и реальные адреса собранного сайта (`grep`/`curl` по
`site_dir`/живому сайту) — «сборка прошла» и «правильный URL открывается» — разные проверки,
и первая не гарантирует вторую.
