...
TITLES = {
    ...
    "manual-2-etap/05b-what-not-to-label.md": "Что не размечаем / Битое",
...
#
# \d{2}[a-z]?- — а не просто \d{2}-: единственный файл мануала с буквенным суффиксом в номере,
# "05b-what-not-to-label.md", иначе не матчился вовсе (буква перед дефисом ломала \d{2}-) — ссылки
# на него текстом-именем файла молча оставались нетронутыми, без ошибки и без предупреждения сборки.
_LINK_RE = re.compile(
    r"\[(?:\.\./)?(?:manual-[23]-etap/)?\d{2}[a-z]?-[a-z0-9-]+\.md(?:#[^\]]*)?[^\]]*\]\(([^)]+)\)"
    r"(\s+([23])\s*этап[а-я]*)?"
)
