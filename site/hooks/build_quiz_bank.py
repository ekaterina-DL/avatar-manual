...
Правило про сегмент в видео-вопросах (источник A, «Определите по видео: <поле>»): в тексте
вопроса всегда указан оцениваемый сегмент. Значение классификатора относится к КОНКРЕТНОМУ
сегменту, а в одном видео сегментов может быть несколько — с разными значениями полей, —
поэтому без указания отрезка вопрос неоднозначен. Отсюда:
* подпись с тайм-кодом ((78.4–95.9), (0:06.48–0:18.06), (0–86.3), (до 0:53)) → в вопросе
  «Оцениваемый сегмент: M:SS–M:SS», плеер играет именно этот отрезок (#t=start,end);
* обрезанный ролик (/trimmed/…__segment_…) — это уже один сегмент целиком → «Оцениваемый
  сегмент — весь ролик»;
