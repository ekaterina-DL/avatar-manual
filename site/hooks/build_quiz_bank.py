...
Правило про сегмент в видео-вопросах (источник A): классификатор заполняется для
КОНКРЕТНОГО сегмента, а в одном видео сегментов может быть несколько — с разными
значениями полей, — поэтому вопрос должен говорить про сегмент, а не про «видео», и
указывать, какой именно (просьба заказчика после тестирования). Формулировка:
«Определите значение поля «<Поле>» для сегмента M:SS–M:SS.». Отсюда:
* подпись с тайм-кодом ((78.4–95.9), (0:06.48–0:18.06), (0–86.3), (до 0:53)) → «для
  сегмента M:SS–M:SS», плеер играет именно этот отрезок (#t=start,end);
* обрезанный ролик (/trimmed/…__segment_…) — это уже один сегмент целиком → «для этого
  сегмента (ролик целиком)»;
* полный видео-источник без тайм-кода в подписи → вопрос НЕ собирается (нельзя честно
  указать, какой отрезок оценивать).
Для вопросов про «Битое» (источники B, C-антипримеры) тайм-код не нужен: бракуется всё
видео целиком, отдельный сегмент там не оценивается.
...
_NUMBER_RECIPES = [
    ...
    {
        # Число 20 минут (с 25.09.2026; было 10:15) живёт на 05b-what-not-to-label.md
        # (раздел «Другое»), а не на 02-segments.md — 02-segments.md его не содержит.
        "page": "manual-2-etap/05b-what-not-to-label.md",
        "confirm": r"20 минут",
        "question": "Какова максимальная длина исходного видео, которое ещё берём в работу (с 25.09.2026)?",
        "correct": "20 минут",
        "wrong": ["10:15", "15 минут", "30 минут"],
        "anchor": "другое",
...
def _segment_phrase(start, end, trimmed, url=""):
    ...
    if start is not None:
        return f"сегмента с {_fmt_tc(start)} до конца ролика"
    return "этого сегмента"
