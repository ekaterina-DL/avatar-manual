...
MAPPINGS = [
    ...
    },
    {
        ...
        # Все 3 видео новые, найдены 20.08.2026 в "АВАТАР 3 ЭТАП.pdf". Первые два (перепад
        # серый/светло-серый) — те же самые ролики, что уже используются в мануале 2 этапа
        # (04-classifier.md, поле "Фон", значение "Нейтральный/размытый") — кросс-мануальное
        # переиспользование (разные этапы, разные классификаторы, разные поля) не подпадает под
        # правило "одно видео — одно место" (оно про дубли внутри одного мануала).
        # Каждый пример — отдельным пунктом (см. комментарий у поля 3 выше про минимум 2 пункта-
        # видео для сборки .video-block группировщиком).
        "target_file": "manual-3-etap/01-classifier.md",
        "anchor": "### 12. 📷 Смена ракурса/зум/панорамирование во время речи",
        "position": "before_line",
        "items": [
            "- **solid (склейка обоев не мешает считать фон однотонным):**\n"
            "  [пример](https://gigaeye-kandinsky-spark.obs.ru-moscow-1.hc.sbercloud.ru/ak/avatar/c812c9a7-534c-4e5c-9b3d-61dddb237d76/trimmed/-194055153_456239189__segment_1_6_31.mp4)",
            "- **solid (перепад серый/светло-серый — пример 1, всё ещё однотон):**\n"
            "  [пример](https://gigaeye-kandinsky-spark.obs.ru-moscow-1.hc.sbercloud.ru/ak/avatar/1f58db0b-da16-4b3c-80fe-1d647ee5c666/trimmed/-215695205_456239021__segment_1_17_37.mp4)",
            "- **solid (перепад серый/светло-серый — пример 2, всё ещё однотон):**\n"
            "  [пример](https://gigaeye-kandinsky-spark.obs.ru-moscow-1.hc.sbercloud.ru/ak/avatar/1f58db0b-da16-4b3c-80fe-1d647ee5c666/trimmed/-214391033_456239025__segment_1_0_92.mp4)",
        ],
        "max_items": 3,
    ...
    },
    {
        ...
        # Все 3 видео новые, найдены 20.08.2026 в "АВАТАР 3 ЭТАП.pdf". Второе (49-я секунда) —
        # тот же ролик, что уже используется в поле 6 выше (там — про пропущенную склейку, здесь
        # — про то, что мелкий посторонний звук можно игнорировать): разные комментарии под
        # разные поля, разрешённый случай правила.
        "target_file": "manual-3-etap/01-classifier.md",
