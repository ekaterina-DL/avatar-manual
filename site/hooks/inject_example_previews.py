...
MAPPINGS = [
    ...
    {
        # Пункт "Ракурс" в банке примеров — про поле "Преобладающий ракурс", которое (после
        # дедупа полей классификатора, 12.08.2026) идёт прямо перед "Освещение": анкорим на
        # начало следующего поля, а не на последнюю строку самого "Преобладающего ракурса" — та
        # строка переносится и заканчивается многострочной цитатой в backtick-теге, и вставка
        # ровно "после неё" рвала бы code-span цитаты пополам, если бы граница пришлась на
        # середину тега.
        "target_file": "manual-2-etap/04-classifier.md",
        "anchor": "- **Освещение:**",
        "position": "before_line",
