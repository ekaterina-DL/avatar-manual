...
MAPPINGS = [
    ...
    {
        # Пункт "Ракурс" в банке примеров — про поле "Преобладающий ракурс", которое идёт прямо
        # перед "Освещение": якоримся на заголовок "### Освещение", а не на последнюю строку
        # самого "Преобладающего ракурса" — та строка переносится и заканчивается многострочной
        # цитатой в backtick-теге, и вставка ровно "после неё" рвала бы code-span цитаты
        # пополам, если бы граница пришлась на середину тега.
        "target_file": "manual-2-etap/04-classifier.md",
        "anchor": "### Освещение",
        "position": "before_line",
        "source_file": "manual-2-etap/11-example-library.md",
