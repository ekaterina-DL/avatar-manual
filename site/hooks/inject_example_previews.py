...
MAPPINGS = [
    {
        ...
        "anchor": "- **Темп речи** (Медленный / Быстрый)",
        "position": "after_line",
    ...
    {
        # Пункт "Ракурс" в банке примеров — про поле "Преобладающий ракурс" (Колонка 1), а не
        # "Группа данных" (Колонка 3): после дедупа полей по колонкам классификатора (12.08.2026)
        # эти поля больше не соседствуют в тексте страницы. Якорь — заголовок "Колонка 2" (не
        # последняя строка поля "Преобладающий ракурс" самого по себе): та строка переносится и
        # заканчивается многострочной цитатой в backtick-теге, и вставка ровно "после неё" рвала
        # бы code-span цитаты пополам, если бы граница пришлась на середину тега.
        "target_file": "manual-2-etap/04-classifier.md",
        "anchor": "### Колонка 2",
        "position": "before_line",
        "source_file": "manual-2-etap/11-example-library.md",
    ...
    {
        # Аналогично — превью "Диалоги и закадровый голос" относится к полю "Тип речи", которое
        # после дедупа полей по колонкам (12.08.2026) больше не стоит перед "Освещение".
        "target_file": "manual-2-etap/04-classifier.md",
        "anchor": "- **Тип речи** (Монолог (подготовленный)",
        "position": "before_line",
        "source_file": "manual-2-etap/11-example-library.md",
