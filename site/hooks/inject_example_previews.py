...
MAPPINGS = [
    ...
    },
    {
        ...
        # 4 (не 2): секция-источник теперь содержит 4 подраздела (подготовленный / спонтанный /
        # диалог / закадровый голос — ошибка) — нужны все 4, иначе видео-примеры подтипов
        # монолога ("подготовленный" vs "спонтанный") не попадут в превью на странице
        # классификатора, а именно по ним была путаница у пользователя 20.08.2026 (превью
        # показывало только "Диалог" и "должно быть Монолог" без уточнения подтипа).
        "max_items": 4,
        "label": "Тип речи",
    },
    {
