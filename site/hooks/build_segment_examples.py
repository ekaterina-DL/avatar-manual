...
# <video.*?</video> — добавлено 02.09.2026: локальный .mp4 (embed_local_media.py оборачивает
# его в <video>, а не <iframe>) должен опознаваться карточкой наравне со встроенным VK/YouTube
# — раньше распознавался только <iframe>, и не-iframe контент внутри этих двух разделов молча
# "проглатывался" в подпись ПРЕДЫДУЩЕЙ карточки (сам regex не матчился, блок не начинался).
