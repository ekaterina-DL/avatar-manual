...
def _tempo_test_mapping():
    ...
    что и у "Тип речи"/"Эмоции" раньше — для manual-2-etap этот приём применён уже трижды, все
    видео из 11-example-library.md для 04-classifier.md теперь зашиты как "items"). Сам механизм
    "source_heading + усечение по max_items + ‘→ ещё N’" по-прежнему используется другими
    реальными записями (например, для manual-3-etap/07-example-library.md), поэтому держим
    регрессии живыми через mapping, независимый от реального MAPPINGS — тот же приём, что и у
    _rakurs_test_mapping() ниже."""
    return {
...
def test_mapping_with_hardcoded_items_skips_source_file():
    ...
    remaining == 0, так что счётной ссылки "ещё N примеров" не появляется. До 03.09.2026 здесь
    вместо неё ставилась общая ссылка-приглашение на страницу "Банк примеров" целиком — но у
    такого mapping своего раздела там нет вообще, так что это была неподтверждённая ссылка
    ("там реально есть что-то ещё"), а пользователь потребовала, чтобы любая отсылка на банк
    примеров была подтверждена. Теперь для таких mapping никакой ссылки не ставится вовсе."""
    from inject_example_previews import _apply_mapping
    ...
    assert "больше примеров" not in result
...
def test_mapping_table_has_label_for_every_entry():
    ...
    expected_labels = {
        ...
        "Насколько хорошо видно лицо говорящего",
        "Использует ли персонаж микрофон или телефон",
        "Непрерывная ли сцена",
        "Количество говорящих",
        "Субтитры/текст",
        "Характеристика фона",
        "Смена ракурса/движение камеры",
        "Активность движения",
        "Законченность фразы",
        "Посторонние шумы",
        "Вид съёмки",
