...
Видеоразбор реальных ошибок целиком — см. [«Полезные материалы»](00-overview.md#полезные-материалы).
...
### Объём и поза тела (руки) {: .field-label-heading }

Самая частая причина, примерно каждая третья ошибка классификатора: не отмечены руки, когда
кисти видны в кадре, либо наоборот — отмечены, хотя рук не видно.
...
### Тип речи: Монолог/Диалог {: .field-label-heading }

Вторая по частоте причина: диалог путают с монологом и наоборот.
...
### Фон: Динамичный/уличный {: .field-label-heading }

Съёмка на улице сама по себе требует значения «Динамичный/уличный», независимо от движения в
кадре. Так же ставится «Динамичный/уличный» при **любом движении** за спиной говорящего (за окном,
прошёл человек или животное и т.п.) — см. [«Фон»](04-classifier.md#6-фон).
...
### Эмоции {: .field-label-heading }

Определяются по интонации и тону голоса, а не по мимике/выражению лица (см.
...
### Пол {: .field-label-heading }

Поле с объективно проверяемым ответом; заказчик реагирует на такие ошибки особенно болезненно
(см. [04-classifier.md](04-classifier.md#1-пол)).
...
### Ракурс, Язык, Темп речи и Возраст {: .field-label-heading }

Реже встречаются ошибки, но принцип тот же: значение должно быть одно и соответствовать тому,
что реально видно/слышно на видео, а не меняться произвольно между сегментами одного человека.
...
💡 Чтобы поймать именно такую ошибку до отправки, используйте **кнопку «Проиграть фрагмент
видео»** — она проигрывает только сам выделенный сегмент, что делает случайно попавшую склейку
или смену кадра гораздо заметнее. Подробности — в
[Сегменты](02-segments.md#как-выделять-сегменты-в-интерфейсе).
