...
<span class="field-label">Объём и поза тела (руки)</span> — самая частая причина, примерно
каждая третья ошибка классификатора: не отмечены руки, когда кисти видны в кадре, либо наоборот
— отмечены, хотя рук не видно.
...
<span class="field-label">Тип речи: Монолог/Диалог</span> — вторая по частоте причина: диалог
путают с монологом и наоборот.
...
<span class="field-label">Фон: Динамичный/уличный</span> — съёмка на улице сама по себе требует
значения «Динамичный/уличный», независимо от движения в кадре.
...
<span class="field-label">Эмоции</span> — определяются по интонации и тону голоса, а не по
мимике/выражению лица (см. [04-classifier.md](04-classifier.md#7-эмоции-и-выражение-лица)).
...
<span class="field-label">Пол</span> — поле с объективно проверяемым ответом; заказчик реагирует
на такие ошибки особенно болезненно (см. [04-classifier.md](04-classifier.md#1-пол)).
...
<span class="field-label">Ракурс, Язык, Темп речи и Возраст</span> — реже встречаются ошибки, но
принцип тот же: значение должно быть одно и соответствовать тому, что реально видно/слышно на
видео, а не меняться произвольно между сегментами одного человека.
