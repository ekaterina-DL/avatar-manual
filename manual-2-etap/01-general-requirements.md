...
⚠️ **Контекст (важно понимать, почему этого нет в официальной инструкции):** заказчик подтвердил
(02.09.2026), что сбор мультиков/антропоморфов был **разовым заданием** по отдельной просьбе
заказчика, а не постоянной частью проекта — «в инструкции про них нет и не будет». То же самое
касается видео с **нейро-аватарами**, которые специально собирались один раз для калибровки
(при этом само правило про исключение AI-генераций из обычной разметки остаётся постоянным, см.
[«Что не размечаем»](05b-what-not-to-label.md)). Поэтому все правила выше по антропоморфам
взяты из дополнительной переписки/ОС, а не из основного ТЗ — это ожидаемо, а не пробел.
