...
**Как считается процент:** на валидацию попадает случайная выборка — **~30%** от заданий, которые
выполнил асессор. Итоговый % = (верно выполненных в этой выборке) / (всего заданий в выборке).
Дальше этот процент проставляется у **всех** заданий асессора в пуле — в том числе у тех, что в
выборку не попали и вообще не проверялись. Поэтому даже 1–2 ошибки в небольшой случайной выборке
заметно двигают итоговый %, даже если по всему объёму сделанного доля ошибок невелика.
