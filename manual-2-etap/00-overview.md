...
На 2 этапе действует общий норматив качества — 85%.

**Как считается процент:** на валидацию попадает случайная выборка заданий, которые выполнил
асессор.

<div class="quality-formula" markdown="1">
<span class="qf-label">Итоговый % =</span>
<span class="qf-frac"><span class="qf-num">верно выполненных в выборке</span><span class="qf-den">всего заданий в выборке</span></span>
</div>

Этот процент относится ко **всем** заданиям асессора в конкретном пуле — в том числе к тем, что в
выборку не попали и вообще не проверялись. Из-за этого небольшое число ошибок в выборке может
заметнее повлиять на итоговый %, чем кажется по общему объёму сделанной работы — это особенность
случайной выборки, а не сигнал, что старания не учитываются. Ориентируйтесь на комментарии
валидатора и динамику, а не на разовое колебание процента.
...
случайной выборке из заданий асессора (см. выше «Как считается процент»); он оставляет комментарии
