...
**Как считается %:** не по всем вашим заданиям в пуле, а по выборке из них, которую проверяет
валидатор — поэтому даже 1–2 ошибки в этой выборке могут заметнее повлиять на итоговый %, чем
кажется по общему объёму сделанной работы. Если ваш % вызывает вопросы — обратитесь к руководителю
группы; комментарии валидатора и общая динамика важнее разового колебания процента.
...
выборке из заданий асессора (см. выше «Как считается %»); он оставляет комментарии
