...
проверяемым ответом, а не оценочные (в отличие, например, от освещения): заказчик проверяет их
так же строго, как границы сегмента.
...
- **Голова** — в кадре видна только голова человека (с шеей или без неё); плечи не видны.
- **Голова и плечи** — видны голова и плечи (достаточно хотя бы одного плеча), человека видно
  по грудь/пояс; руки/кисти рук в кадре не видны — это касается и случая, когда в кадр попадают только кончики пальцев
...
⚠️ **Логика правила — объём тела, а не количество коленей в кадре.** Дело не в том, видно ли
одно колено или оба, а в том, видна ли **бедренная часть ноги**. Та же логика — что для стоящего,
что для сидящего человека: если бёдра не видны, а видна только коленка (или даже обе) — это
по-прежнему «Голова, плечи и руки» (человек виден по грудь), а не «полный рост».
`[Разметка ВК видео — ОС 2 этап, 03.09.2026]`

⚠️ Если **пальцы человека попали в кадр буквально на долю секунды** (кисти рук при этом
полностью не видны) — ставьте «Голова и плечи».
`[Комментарии для мануала ко 2 этапу Аватара, 04.09.2026]`

⚠️ **Граница между «Голова» и «Голова и плечи».** «Голова» ставится, только когда видна одна
голова и шея (или голова без шеи). Если в кадре видно **хотя бы одно плечо** — пусть оно
обрезано краем кадра и видно не до локтевого сустава — ставьте уже «Голова и плечи». На практике значение «Голова» встречается **очень редко**.
`[Разметка ВК видео — ОС 2 этап, вердикт заказчика, 21.09.2026; Ответы на вопросы 2 этап,
21.09.2026]`
...
- [**Голова и плечи**, а не «Голова» — в кадре видно одно плечо (12.9–30.0)](https://gigaeye-kandinsky-spark.obs.ru-moscow-1.hc.sbercloud.ru/kandi_de_team/post_train/pre_stage/video/avatars/2e27a696-5f02-4d90-9db5-893ba5a32136/downloaded_raw/kicq8xb_Uq8.mp4)
- [**Голова и плечи** — пальцы мелькнули на долю секунды, кисти не видны (2.05–36.2)](https://gigaeye-kandinsky-spark.obs.ru-moscow-1.hc.sbercloud.ru/kandi_de_team/post_train/pre_stage/video/avatars/2e27a696-5f02-4d90-9db5-893ba5a32136/downloaded_raw/ugOTJA0NGAQ.mp4)
- [**Голова и плечи** — руки в кадре ни разу не появились (0.6–15.1)](https://gigaeye-kandinsky-spark.obs.ru-moscow-1.hc.sbercloud.ru/kandi_de_team/post_train/pre_stage/video/avatars/aad81115-c102-4001-8e6a-67207c05ec73/downloaded_raw/RkGxPqM_JOE.mp4)
- [**Голова и плечи** — руки не видны (63.5–78.3)](https://gigaeye-kandinsky-spark.obs.ru-moscow-1.hc.sbercloud.ru/kandi_de_team/post_train/pre_stage/video/avatars/2e27a696-5f02-4d90-9db5-893ba5a32136/downloaded_raw/v-I9QfdvGvA.mp4)
...
`[Журнал проверок ОС; Разметка ВК видео — ОС 2 этап, 21.09.2026]`

**Скриншоты для наглядности — «Голова» и «Голова и плечи»:**

<!-- video-eyebrow: 3. Объём и поза тела — скриншоты -->
...
- [Человек постоянно крутит головой (с 8 сек — два основных ракурса, полуоборот и профиль) — выбираем только один](assets/o0g3jYZ_ORM.mp4)

`[Журнал проверок ОС; Инструкция заказчика, 25.09.2026]`
...
- **Естественный, но статичный** — обычная бытовая обстановка (комната, студия
  телепрограммы, школа и т.п.), в которой **за спиной ничего не движется**; неподвижная тень.
- **Динамичный/уличный** — за спиной говорящего есть **любое движение** (оно не должно
  отвлекать от лица), в отражении (зеркало, стекло) присутствует движение, движущаяся/мерцающая
  тень.
...
⚠️ **Одна лишь стена — ещё не улица.** Такая стена может быть и в помещении, поэтому по ней
одной «Динамичный/уличный» не ставим. Признаки улицы и динамики — деревья, проезжающий
транспорт, пролетающие птицы и т.п. **Ориентируйтесь только на границы выделяемого
сегмента:** если внутри выбранных границ нет ничего, что указывало бы на улицу или движение,
ставьте «Естественный, но статичный».
`[Разметка ВК видео — ОС 2 этап, вердикт заказчика, 21.09.2026]`

⚠️ **На 2 этапе любое движение за спиной говорящего — это динамика.** Даже слабое или
короткое: за окном ветер и двигаются деревья, флаг или что-либо ещё; на фоне хотя бы один раз за
5 минут прошла собака или человек; колышутся листва или ткань. Ничего, что хоть немного
шевелится, в «статику» не пропускаем — фон будет «Динамичный/уличный». На 3 этапе критерии
немного другие, см.
[мануал 3 этапа](../manual-3-etap/01-classifier.md#11-характеристика-фона-за-спиной-говорящего).
`[Ответы на вопросы 2 этап, 21.09.2026]`

⚠️ Категория **«Динамичный/уличный» — особенность именно 2 этапа**: сам факт съёмки на
...
⚠️ Если фон размыт **до практически неразличимых пятен** — ставьте «Нейтральный/размытый». Если
фон в **расфокусе** (резкость наведена на лицо), но **обстановка всё равно узнаваема** —
классифицируйте по тому, что реально видно за спиной («Естественный, но статичный» или
«Динамичный/уличный», смотря по ситуации), а не как «Нейтральный/размытый» — эта категория
именно про однотонный/замыленный фон по своей природе, а не про любой случайный расфокус
камеры.
`[Комментарии для мануала ко 2 этапу Аватара, 04.09.2026]`
...
- **Нейтральные/спокойные** — ровный тон голоса и выражение лица без выраженной
  ...
  положительной или серьёзной/негативной эмоции). Ставим, только если эмоции
  **практически отсутствуют**.
- **Положительные** — радость, улыбка, интерес.
- **Серьёзные/сосредоточенные/негативные** — удивление, сомнение, грусть; сюда же относятся
  **злость и явная напряжённость**, когда по человеку видно, что тема его беспокоит, «трогает
  за живое», раздражает или злит.

`[Инстр. Kandinsky-Аватар, стр.4; Памятка «Аватар 2 этап», стр.5; Ответы на вопросы 2 этап,
21.09.2026]`
...
- [Человек улыбается/смеётся — эмоция положительная](assets/hYRf9bHd7Ic__segment_0479338_4206752.mp4)
- [Человек улыбается, но интонация не положительная — эмоция ставится по голосу, а не по улыбке](assets/BUQLVpsqn88.mp4)
...
⚠️ **То же самое — для пения.** Тип речи считается по количеству людей, от которых реально
исходит звук (поют/говорят), а не по тому, отвечают ли они друг другу репликами — у пения,
в отличие от разговора, обычно и не бывает вопросов-ответов. **Поёт один человек** — это
**монолог** (даже если рядом молча присутствует ещё кто-то), а **поют двое и более** (дуэтом,
хором, по очереди) — уже **диалог**, даже если они не «переговариваются» между собой в
привычном смысле, а просто поют вместе. `[Уточнение пользователя, 16.09.2026]`
...
- [Говорит на камеру один человек — монолог](assets/Ob8ICsKhsTc__segment_38758126_78550061.mp4)
- [Один говорит, второй «поддакивает» — это уже диалог](assets/xee6f8dBB58__segment_0312614_74402193.mp4)
...
⚠️ **Оценивайте темп речи на обычной скорости просмотра**, без ускорения/замедления. Это
довольно субъективный параметр, и валидаторы не проверяют его слишком строго — но если
очевидно, что человек говорит быстро (например, испанский/португальский язык), а стоит
«Медленный», это будет засчитано как ошибка.
`[Комментарии для мануала ко 2 этапу Аватара, 04.09.2026]`
...
⚠️ **Названия и имена собственные**, которые невозможно естественно произнести на
преобладающем языке (бренды, топонимы, имена людей и т.п.), **в долю «второго языка» не
засчитываются** — даже если таких слов в сегменте несколько, это не переключение языка, а
непереводимое слово/название.
`[Разметка ВК видео — ОС 2 этап, 03.09.2026]`
...
- [Английский язык](assets/-94892022_171336072__segment_1_31_42__seg1.mp4)
- [Другой язык](assets/CZgTL3KqHtg__segment_23111611_34403896.mp4)
...
- **Один человек в кадре** — в выбранном сегменте виден один человек (независимо от того,
  говорит ли он). Люди на заднем плане (мимо проходящие, мелькнувшие вдали) не считаются.
- **Два и более человека в кадре** — в выбранном сегменте видно двух и более людей из тех, кого
  снимает камера: спикеров и тех, кто стоит рядом со спикером (в том числе молча).

`[Памятка «Аватар 2 этап», стр.6; Инструкция заказчика, 04.09.2026]`

⚠️ **Людей на фоне не отмечаем.** Это одна из самых частых ошибок в реальных проверках.
Считаем только того, кого снимает камера: главного спикера и тех, кто стоит с ним рядом в
кадре. Мимо проходящие и мелькающие на фоне люди не в счёт — если кроме спикера на фоне лишь
мелькнул кто-то ещё, ставьте «Один человек в кадре».
`[Разметка ВК видео — ОС 01.09.2026; Разметка ВК видео — ОС 2 этап, 21.09.2026]`

⚠️ Поле проставляем по людям, которых снимает камера, **независимо от того, кто из них
говорит/поёт.** `[Чат исполнителей, 11.08.2026]`
...
**Пример:** [главный объект один, говорит в камеру — «Один человек в кадре»](assets/wcBashWJQSs.mp4).
`[Инструкция заказчика, 25.09.2026]`
...
**Примеры на разделение речи и пения по отдельным сегментам:**

- [Пример 1](assets/iSor3Id1zOw__segment_1_15_38__seg1.mp4)
- [Пример 2](assets/ugSJE8B4kvs__segment_3621873_52414706.mp4)
