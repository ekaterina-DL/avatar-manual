...
⚠️ **Внутри классификатора поля неравнозначны по строгости.** Часть полей заказчик прямо
признаёт оценочными (например, освещение), единого порога согласованности по ним нет — за них
не «наказывают» так строго. А вот **пол и язык** (см. также [«Язык и акценты»](#10-язык-и-акценты)
ниже) — поля с объективно проверяемым правильным ответом, не «оценочные», и по практическому
опыту команды заказчик регулярно и болезненно реагирует именно на ошибки в них: «неверный пол»
отдельно назван в списке частых ошибок экзамена наравне с границами сегмента и пропущенными
склейками/водяными знаками (см. [«Частые ошибки на экзамене»](00b-exam.md#частые-ошибки-на-экзамене))
— то есть фактически стоит в одном ряду с «грубыми» ошибками, а не с типично оценочными полями
классификатора. Примеры ошибок — [«Частые ошибки»](06-common-mistakes.md#1-ошибка-классификатора).
...
[мануал 3 этапа](../manual-3-etap/01-classifier.md#поля-классификатора-19).
`[Разметка ВК видео — ОС 20.05 СТ2]`
...
Почта. ОС по Заявкам 26-28, 24.07.2026]`
