...
- **Потеря лица** (не из-за перекрытия чем-либо, а из-за поворота головы) — недопустима вне
  зависимости от длительности: если человек повернул голову настолько, что нос «уходит за угол»
  и лицо целиком перестаёт быть видно, такой момент нельзя включать в сегмент. Это ещё не
  «профиль» (силуэт носа сбоку всё ещё виден — профиль допустим) — граница между ними разобрана в
  [«Преобладающий ракурс»](04-classifier.md#4-преобладающий-ракурс); что делать с сегментом в
  таком случае — см. [«Когда разделить на отдельные
