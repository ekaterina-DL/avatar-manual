...
- **С 25.09.2026 максимальная длина исходного видео — 20 минут** (было 10:15) — не размечаем
  видео длиннее этого порога, в т.ч. по видео из заявки 28, где исходные видео имеют длину от
  420 сек до ∞.

`[Инстр. Kandinsky-Аватар, стр.2, 7-10; Памятка «Аватар 2 этап», стр.3; Инструкция заказчика,
25.09.2026]`
...
плеер (чёрная полоска снизу, скруглённые углы по краям).

*Скругление углов* — особенность интерфейса плеера, а вот *прямая узкая полоска понизу кадра* —
часть исходного видео, настоящий брак.
...
Больше примеров — см. [«Банк примеров»](11-example-library.md#большая-пиксельность-19).
`[Вопросы по проекту (1-2ч) — Чат исполнителей, февраль 2026, мнение исполнителей]`

⚠️ **При активных движениях человека в кадре небольшая пиксельность допустима.** Если персонаж
очень динамичен (быстро поворачивает голову, много двигается), полностью избежать пиксельности
невозможно — пока черты лица и губы остаются читаемыми, это не брак. Такое видео не нужно
сразу отправлять в «Битое»: найдите участок, где лицо читается, и разметьте его.
...
### Тряска камеры {: .field-label-heading }

- Сегмент не подходит только если **движение камеры настолько резкое**, что влияет на чёткость
  изображения лица человека (появляется размытость/смазанность картинки) — тогда это «Битое».
  Типичный пример такой ситуации — съёмка на бегу («если человек бежит и что-то снимает»).
- **Плавное или в меру активное «гуляние» камеры**, которое не мешает чётко видеть лицо, — это
  норма, даже если камера не абсолютно статична.
...
### Пережатие {: .field-label-heading }

Пережатие (компрессионные артефакты) — особенно заметно на руках и губах — на всём
протяжении сегмента делает его непригодным. `[ОС заказчика, 17.07.2026]`

</div>

💡**Как распознать визуально:** видео начинает *«полосить»* — при каждом движении говорящего
(лицо, руки, всё, что движется) появляются *чёрные горизонтальные полосы* — это брак, такое
видео отправляется в «Битое». `[Ответы на вопросы по Аватару, 02.09.2026; Разметка ВК
видео — ОС 01.09.2026]`
...
![Стоп-кадр из неочевидного примера пережатия: при увеличении на руках и зубах видны сильные горизонтальные полосы](assets/perezhatie-nfjovty-frame.jpeg)

⚠️ Пример не такой очевидный, как кажется, но при увеличении изображения можно увидеть сильные
полосы на руках, на зубах. Стоит обратить внимание на необходимость тщательного, а не беглого
просмотра таких дефектов.
...
- [❌ Отсутствие синхронизации аудио и видео](https://gigaeye-kandinsky-spark.obs.ru-moscow-1.hc.sbercloud.ru/ak/vk/05_02_2026/-213074806_456239605/-213074806_456239605.mp4).

`[Комментарии к мануалу от заказчика, 06.09.2026 и 08.09.2026]`

<div class="field-label-row" markdown="1">

### Смена кадра/сцены {: .field-label-heading }

Смена кадра/сцены внутри сегмента — брак, если из-за неё **человек исчезает из кадра**: сегмент
должен показывать говорящего человека непрерывно, без пропаданий из поля зрения камеры.
...
### Потеря лица {: .field-label-heading }

**Потеря лица** (не из-за перекрытия чем-либо, а из-за поворота головы) — недопустима вне
зависимости от длительности: если человек повернул голову настолько, что нос «уходит за угол» и
лицо целиком перестаёт быть видно, такой момент нельзя включать в сегмент. Это ещё не «профиль»
(силуэт носа сбоку всё ещё виден — профиль допустим) — граница между ними разобрана в
[«Преобладающий ракурс»](04-classifier.md#4-преобладающий-ракурс); что делать с сегментом в таком
случае — см. [«Когда разделить на отдельные
сегменты»](02-segments.md#когда-разделить-на-отдельные-сегменты).
`[Встреча с новичками по 2 этапу, 29.07.2026]`
...
- **Замедленное или ускоренное видео** — недопустимо, независимо от степени изменения
  скорости. `[Инструкция заказчика, 04.09.2026]` Реальный сложный случай — [ускоренная речь,
  которую легко принять за обычную быструю манеру говорить](assets/-W3dLjDtKIA.mp4): у
  некоторых народов и языков естественный темп речи очень высокий, поэтому ускорение видео
  можно не заметить с первого раза; при повторном просмотре оно становится очевидным.
  `[Разметка ВК видео — ОС 2 этап, 05.09.2026; Инструкция заказчика, 25.09.2026]`
...
💡 **Практический ориентир при принятии решения на глаз:** важна именно скорость движения
камеры, а не сам факт, что она двигается. Резкий скачок камеры внутри сегмента приравнивается к
склейке — см. [«Склейки»](#склейки) в разделе «Дефекты, которых не должно быть в сегменте».

<div class="field-label-row" markdown="1">

### Молчание {: .field-label-heading }
...
### Закадровый голос {: .field-label-heading }
...
⚠️ Исключение — **короткие односложные звуки** вроде «угу» или «м»: это просто звук, а не
закадровый голос, видео из-за них в «Битое» не отправляем. Слова и реплики закадрового человека
по-прежнему недопустимы.
`[Разметка ВК видео — ОС 2 этап, 21.09.2026]`

**Пример ошибки:** [закадровый голос попал в начало сегмента](assets/0CRFA6w81ME__segment_12841482_50355926.mp4).
`[Инструкция заказчика, 25.09.2026]`

<div class="field-label-row" markdown="1">

### Перекрытие рта {: .field-label-heading }

На **1–2 секунды** (почесать нос, поправить очки и т.п.) — **допустимо**, но только если это
**не происходит в момент речи**, сегмент из-за этого обрезать не нужно.
`[Ответы на вопросы по Аватару, 02.09.2026]`
...
если рот перекрыт рукой/предметом именно тогда, когда человек **в этот момент говорит** — такой
участок в сегмент **включать нельзя**, даже если он короткий;
`[Ответы на вопросы по Аватару, 02.09.2026]`
...
границ сегмента** (исключить проблемный участок).
...
### Перекрытие лица {: .field-label-heading }

То же самое правило работает и для **перекрытия лица целиком** (не только рта): короткий эпизод
перекрытия внутри длинного сегмента можно оставить как есть, но если участок с перекрытым лицом
длится долго — сегмент нужно разделить на два (до и после), а не выделять одним куском.
`[Разметка ВК видео — ОС 29.05 СТ2]`

</div>

⚠️ **С 25.09.2026** — как правило, сегменты, где **черты лица перекрыты микрофоном или любым
другим предметом**, не выделяем. Отдельный нюанс про **широкий микрофон** (например, при
профессиональной записи вокала в студии): если он закрывает часть щеки/подбородка, но не сами
губы, а артикуляция губ и мимика всё равно хорошо видны — это не считается перекрытием рта, такой
сегмент ***берём в разметку***.
`[Видео с разбором ошибок — Аватар 2 этап; Инстр. Kandinsky-Аватар, стр.2; Инструкция заказчика,
25.09.2026]`

<div class="field-label-row" markdown="1">

### Очки {: .field-label-heading }

**Солнцезащитные очки** — допустимы, кроме случаев, когда через них совсем не видно глаз у
...
⚠️ Модели для обучения не нужны говорящие люди с закрытыми/невидимыми глазами — поэтому если
...
Даже если **очки прозрачные и глаза видны** — в них ***не должно быть*** отражений (бликов,
отражённого света и т.п.), закрывающих обзор на глаза.
`[Ответы на вопросы по Аватару, 02.09.2026]`

⚠️ Для обучения модели важно лицо, поэтому **должны быть хорошо видны оба глаза**: если из-за
бликов один глаз (например, правый) почти не виден — сегмент не подходит, даже когда второй глаз
читается. Если блики держатся на протяжении всего видео и подходящего участка нет — это «Битое».
`[Разметка ВК видео — ОС 2 этап, вердикт заказчика, 21.09.2026]`
...
- [✅ Отражения на очках допустимы — глаза хорошо видны, сегмент подходит](assets/dVqpQtlTh2M__segment_2_108_119__seg2.mp4).
...
- [❌ Блики закрывают глаз (правый глаз почти не виден) — «Битое»](https://gigaeye-kandinsky-spark.obs.ru-moscow-1.hc.sbercloud.ru/kandi_de_team/post_train/pre_stage/video/avatars/2e27a696-5f02-4d90-9db5-893ba5a32136/downloaded_raw/jVcK4ur4Qxw.mp4).
- ![grid: ❌ Тёмные очки скрывают глаза — «Битое»](assets/ochki-temnye-primer.jpeg)
- ![grid: ✅ Прозрачные очки, глаза видны — сегмент подходит](assets/ochki-prozrachnye-primer.jpeg)

`[Комментарии к мануалу от заказчика, 06.09.2026; Разметка ВК видео — ОС 2 этап, вердикт заказчика,
21.09.2026; Инструкция заказчика, 25.09.2026]`

<div class="field-label-row" markdown="1">

### Фоновый шум и ветер {: .field-label-heading }

**Фоновый шум** (ветер, гул, голоса и звуки на фоне) допустим, пока он **не мешает разобрать
речь и не перебивает говорящего**: отчётливо слышно каждое слово, нет «провалившихся» частей
слов. Если шум или громкий голос на фоне заглушает речь — такой участок брать нельзя.
`[Разметка ВК видео — ОС 2 этап, вердикт заказчика, 21.09.2026]`

</div>

⚠️ Одного факта, что на видео слышен шум, недостаточно, чтобы браковать видео целиком: найдите
участок, где речь слышна чисто, и разметьте его. Пример проблемного шума — «Звук ветра слишком
громкий» в [«Банке примеров»](11-example-library.md#битое--примеры-дефектов).

<div class="field-label-row" markdown="1">

### Блики/вспышки/солнечные лучи {: .field-label-heading }
...
### Ракурс спиной {: .field-label-heading }
...
«Битое» вместо выделения сегментов. `[Инстр. Kandinsky-Аватар, стр.1]`

⚠️ ***Перед отправкой в «Битое» важно убедиться***, что во всём видео действительно нет ни
одного подходящего отрезка от 10 секунд — наличие одного «плохого» признака (например, где-то
есть склейка) не значит, что всё видео нужно браковать целиком.
...
⚠️ Это два взаимоисключающих варианта: либо выделяются сегменты, либо ставится «Битое» —
