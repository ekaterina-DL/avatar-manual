...
### Перекрытие/потеря лица {: .field-label-heading }
...
⚠️ **Потеря лица** — отдельный, не менее важный случай: лицо пропадает из виду не из-за
перекрытия чем-либо, а просто из-за поворота головы. Если человек повернул голову настолько, что
нос «уходит за угол» и перестаёт быть виден, — такой момент недопустим, его нельзя включать в
сегмент: нужно искать более подходящий по времени участок или разделить сегмент на два (до и
после поворота). Это ещё не «профиль» (силуэт носа сбоку всё ещё виден — профиль допустим), а
именно момент, когда лицо целиком выходит из кадра по ракурсу — граница между ними подробно
разобрана в [«Преобладающий ракурс»](04-classifier.md#4-преобладающий-ракурс).
`[Встреча с новичками по 2 этапу, 29.07.2026]`
...
[✅ Блики на очках не мешают — сегмент подходит](https://vkvideo.ru/video23364836_456240066)

**Калибровочные примеры (брак):**
